  ...
  ThreadpoolWork *RunThreadpoolWork() {
    ...
    if (state_mesh_ == nullptr) {
      ...
      if (DropIntegerColorWrites(info, num_rtvs, ps_uint_output_registers, ps_sint_output_registers, false)) {
        ...
        state_mesh_ = device_->GetMTLDevice().newRenderPipelineState(info, err);
        if (state_mesh_ != nullptr)
          WARN("Created mesh PSO with mismatched integer color attachment write(s) dropped");
      }
      if (state_mesh_ == nullptr && DropIntegerColorWrites(info, num_rtvs, 0, 0, true)) {
        retried = true;
        state_mesh_ = device_->GetMTLDevice().newRenderPipelineState(info, err);
        if (state_mesh_ != nullptr)
          WARN("Created mesh PSO with writes to integer color attachment(s) dropped");
      }
      if (state_mesh_ == nullptr && retried)
